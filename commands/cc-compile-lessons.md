---
description: Compile .cc-boost/runtime/failures.jsonl into a fresh .cc-boost/runtime/lessons.md. Run periodically (every 50 failures or weekly). Lessons are auto-injected into every UserPromptSubmit by the inject-lessons hook.
...
1. Read `.cc-boost/runtime/failures.jsonl`. If absent or empty, write a stub
   `runtime/lessons.md` with just the header and exit. Tell the user "no failures to
   learn from yet."
...
4. Write the result to `.cc-boost/runtime/lessons.md`. Update the "Last compiled"
...
- Never edit `.cc-boost/runtime/failures.jsonl` — it's append-only.
- Never write any file other than `.cc-boost/runtime/lessons.md`.
